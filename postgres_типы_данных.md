...
### Типы данных для хранения текста и символов:

- character varying(n),
- varchar(n): строка ограниченной переменной длины.
- character(n),
- char(n): строка фиксированной длины.
- text: строка переменной длины без ограничений. Логический тип:
...
- Позволяют хранить набор значений одного типа данных.
- Могут быть одномерными или многомерными.

### UUID:

Уникальный идентификатор. Может быть сгенерирован с помощью функции gen_random_uuid(). Используется для обеспечения
уникальности идентификаторов.

### JSON и JSONB:

Типы данных для хранения и обработки JSON-данных. JSONB представляет собой бинарное представление JSON и поддерживает
индексацию.

### Диапазоны (range types):

Позволяют хранить диапазоны значений, такие как числовые диапазоны или диапазоны дат. Предоставляют операции для
сравнения, поиска и объединения диапазонов.

### Домены:

Пользовательские типы данных, создаваемые на основе существующих типов данных. Позволяют задавать ограничения и правила
для полей таблиц.

### Округление и математические операторы В PostgreSQL предусмотрены функции для округления чисел:

- ceil: округление вверх.
- floor: округление вниз.
- round: округление до ближайшего целого числа.
- trunc: обрезание десятичной части без округления.
- Также в PostgreSQL предусмотрены математические операторы, такие как возведение в степень и извлечение корня.
